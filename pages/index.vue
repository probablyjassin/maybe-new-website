<template>
	...
	<UCard class="mx-auto w-[60%] text-4xl p-5">
		...
		<UCarousel v-slot="{ item }" :items="items" class="carousel">
			<img :src="item.img" width="300" height="400" draggable="false" @click="router.push(item.to)" :style="{ 'view-transition-name': `${item.to.split('/')[2]}`}" />
		</UCarousel>
	</UCard>
	</div>
</template>
...
<script setup lang="js">
	...
	const items = [
	  {"img": '/images/chessboard.webp', "to": "/projects/chessboard"},
	  {"img": '/images/anarchychess.webp', "to": "/projects/anarchychess"},
	]
</script>